...
var MIN_SCALE = 0.6
var MAX_SCALE = 3.667 // 大小档位上限＝原 70 档大小（250基线×3.667 ≈ 917px）
var STEP = 0.1
var CLICK_SQ = 9
...
var FRAME_BASE = '/dsh-whale/frame.png'
var PRESENCE_URL = '/dsh-whale/presence.json'
var CRAWL_SPEED = 45
var CRAWL_DEFAULT_IDLE_MS = 10000

var css = [
  '.dshwv-root{position:fixed;right:0;bottom:0;--dshw-scale:1;--dshw-base:clamp(122px,calc(min(250px,min(100vw,100vh) * 0.28) * var(--dshw-scale)),calc(min(250px,min(100vw,100vh) * 0.28) * 3.667));width:var(--dshw-base);height:var(--dshw-base);pointer-events:none;user-select:none;-webkit-user-select:none;z-index:9999;font-family:inherit;transition:left .16s ease,top .16s ease,transform .3s ease}',
  '.dshwv-root.dshwv-left{transform:scaleX(-1)}',
  '.dshwv-root.dshwv-dragging{cursor:grabbing;transition:none}',
  '.dshwv-root.dshwv-crawl{transition:none}',
...
scaleNumber.addEventListener('input', function () {
  ...
  var s = MIN_SCALE + Math.max(0, Math.min(70, v) - 1) * (MAX_SCALE - MIN_SCALE) / 69
  setScale(s)
})
...
skinSelect.appendChild(soundOpt('a', '方案A (静态)'))
skinSelect.appendChild(soundOpt('b', '方案B (爬行动画)'))
skinSelect.appendChild(soundOpt('original', '方案C (原版)'))
...
remindToggle.title = '回答完毕时若你不在页面上，弹「常回家看看」气泡提醒'
remindToggle.addEventListener('change', function () { setRemindOn(remindToggle.checked) })
...
var crawlToggle = document.createElement('input')
crawlToggle.type = 'checkbox'
crawlToggle.className = 'dshwv-check'
crawlToggle.checked = true
crawlToggle.title = '空闲一段时间没有按压后，鲸鱼会沿屏幕四边爬行'
crawlToggle.addEventListener('change', function () { setCrawlOn(crawlToggle.checked) })
var crawlIdleInput = document.createElement('input')
crawlIdleInput.type = 'number'
crawlIdleInput.min = '1'
crawlIdleInput.step = '1'
crawlIdleInput.className = 'dshwv-number'
crawlIdleInput.value = '10'
crawlIdleInput.title = '空闲多少秒后开始爬行'
crawlIdleInput.addEventListener('input', function () { setCrawlIdle(crawlIdleInput.value) })
crawlIdleInput.addEventListener('change', function () { setCrawlIdle(crawlIdleInput.value) })
var crawlSpeedInput = document.createElement('input')
crawlSpeedInput.type = 'number'
crawlSpeedInput.min = '1'
crawlSpeedInput.max = '1000'
crawlSpeedInput.step = '1'
crawlSpeedInput.className = 'dshwv-number'
crawlSpeedInput.value = '45'
crawlSpeedInput.title = '爬行速度，单位像素/秒'
crawlSpeedInput.addEventListener('input', function () { setCrawlSpeed(crawlSpeedInput.value) })
crawlSpeedInput.addEventListener('change', function () { setCrawlSpeed(crawlSpeedInput.value) })
var row1 = menuRow()
...
rowRemind.appendChild(menuLabel('完成提醒'))
rowRemind.appendChild(remindToggle)
var row9 = menuRow()
...
var row10 = menuRow()
row10.appendChild(menuLabel('空闲爬行'))
row10.appendChild(crawlToggle)
row10.appendChild(menuLabel('空闲'))
row10.appendChild(crawlIdleInput)
row10.appendChild(menuLabel('秒'))
var row11 = menuRow()
row11.appendChild(menuLabel('爬行速度'))
row11.appendChild(crawlSpeedInput)
row11.appendChild(menuLabel('px/秒'))
...
function showBubble() {
  if (!bubbleOn) return
  // 消耗金额/完成提醒泡泡显示期间，余额变动不再弹出普通泡泡
  if (costBubbleActive || remindBubbleActive) return
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  if (gifFadeTimer) { clearTimeout(gifFadeTimer); gifFadeTimer = null }
  bubbleShown = true
...
// —— 完成提醒：回答完毕时如果用户不在页面上（切到别的标签页/窗口），
// 弹「常回家看看」气泡并保持到回来；后台标签同时改标题提示。 ——
var remindBubbleActive = false
var remindPending = false
var remindSavedTitle = null
var REMIND_TEXT = '常回家看看'
function awayFromPage() {
  try {
    return document.hidden === true || document.hasFocus() === false
  } catch (err) { return false }
}
function showRemindBubble(text) {
  if (!bubbleOn) return
  if (costBubbleActive) hideCostBubble()
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  if (costBubbleTimer) { clearTimeout(costBubbleTimer); costBubbleTimer = null }
  if (gifFadeTimer) { clearTimeout(gifFadeTimer); gifFadeTimer = null }
  // 与消耗泡泡一致：取消进行中的余额滚动/延迟计时器，避免竞态覆盖文字
  if (animId) { cancelAnimationFrame(animId); animId = null }
  if (animDelayTimer) { clearTimeout(animDelayTimer); animDelayTimer = null }
  if (settleTimer) { clearTimeout(settleTimer); settleTimer = null }
  remindBubbleActive = true
  bubbleRandomActive = false
  bubbleRandomLines = null
  bubbleShown = true
  lastHintText = null
  ...
  textBox.style.opacity = ''
  bubbleBox.classList.add('dshwv-bubble-open')
  // 刻意不设自动关闭：等用户回到页面或点击气泡
}
function hideRemindBubble() {
  if (!remindBubbleActive) return
  remindBubbleActive = false
  hideBubble()
}
function remindTitleOn() {
  try {
    if (remindSavedTitle === null) remindSavedTitle = document.title
    if (document.title.indexOf(REMIND_TEXT) === -1) document.title = '🐋 ' + REMIND_TEXT + ' · ' + remindSavedTitle
  } catch (err) {}
}
function remindTitleOff() {
  try {
    if (remindSavedTitle !== null) {
      document.title = remindSavedTitle
      remindSavedTitle = null
    }
  } catch (err) {}
}
// 返回 true 表示已由「完成提醒」接管（调用方不要再弹消耗泡泡）
function notifyTurnDone() {
  if (!remindOn) return false
  if (!awayFromPage()) return false
  remindPending = true
  remindTitleOn()
  // 页面可见但没焦点（被别的窗口挡住一半）时直接弹；完全隐藏则等回来再弹
  if (!document.hidden) {
    remindPending = false
    showRemindBubble(REMIND_TEXT)
  }
  return true
}
...
// 把「页面是否聚焦」上报给宿主：宿主据此在回答完成时发 macOS 系统通知
function reportPresence() {
  try {
    var focused = !document.hidden && document.hasFocus()
    fetch(PRESENCE_URL, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ focused: focused }),
    }).catch(function () {})
  } catch (err) {}
}
document.addEventListener('visibilitychange', function () {
  reportPresence()
  if (!document.hidden) onBackToPage()
})
window.addEventListener('focus', function () { reportPresence(); onBackToPage() })
...
function render() {
  // 消耗金额/完成提醒泡泡显示期间，余额渲染不覆盖其内容（金额行/标题行/提示行）
  if (costBubbleActive || remindBubbleActive) return
  var amount, hint
...
function express() {
  ...
  // 爬行时：身体贴着所在的屏幕边缘（肚子朝边缘、头朝行进方向），
  // 通过旋转 img 实现；非爬行沿用吸附朝向（左吸附镜像、右吸附不镜像）
  if (crawling) {
    root.classList.remove('dshwv-left')
    applyCrawlOrientation()
  } else {
...
var scrollGapPx = 17
var crawlOn = true
var crawlIdleMs = CRAWL_DEFAULT_IDLE_MS
var crawlSpeed = CRAWL_SPEED
var skin = 'original'
var frameIdx = 0
var frameLastTs = 0
var frameImages = {}
...
function setRemindOn(v) {
  remindOn = !!v
  remindToggle.checked = remindOn
  saveConfig()
  if (!remindOn) {
    remindPending = false
    hideRemindBubble()
...
function setCrawlOn(v) {
  crawlOn = !!v
  crawlToggle.checked = crawlOn
  crawlIdleInput.disabled = !crawlOn
  saveConfig()
  if (!crawlOn) stopCrawl()
  resetIdleTimer()
}
function setCrawlIdle(v) {
  if (!crawlOn) return
  var n = Math.max(1, Math.round(Number(v) || 0))
  crawlIdleMs = n * 1000
  crawlIdleInput.value = String(n)
  saveConfig()
  resetIdleTimer()
}
function setCrawlSpeed(v) {
  var n = Math.max(1, Math.min(1000, Math.round(Number(v) || CRAWL_SPEED)))
  crawlSpeed = n
  crawlSpeedInput.value = String(n)
  saveConfig()
}
// —— 形象/动画帧：方案C(原版)=单张静态图；A=静态爬行图；B=spritesheet 切片帧序列 ——
var CRAWL_FRAME_COUNT = { a: 8, b: 8 }
var SKIN_ANIMATED = { a: false, b: true } // 方案A 取消爬行动画（爬行时用 crawl-0 静态图）
var CRAWL_ORDER = { b: [5, 0, 3, 7] } // 方案B 指定爬行帧播放顺序（循环）
function frameUrl(sk, type, i) {
  return FRAME_BASE + '?skin=' + sk + '&type=' + type + '&i=' + i
}
function preloadFrames(sk) {
  var types = ['crawl', 'state']
  for (var ti = 0; ti < types.length; ti++) {
    var type = types[ti]
    var max = (type === 'crawl' ? (CRAWL_FRAME_COUNT[sk] || 8) : 5) - 1
    for (var i = 0; i <= max; i++) {
      var key = sk + '/' + type + '/' + i
      if (!frameImages[key]) {
        var im = new Image()
        im.src = frameUrl(sk, type, i)
        frameImages[key] = im
      }
    }
  }
}
function setSkinFrame(type, i) {
  if (skin === 'original') return
  img.src = frameUrl(skin, type, i)
}
// 按压/Q弹/点击时随机切换到一张表情帧（state-1..4，state-0 为待机坐姿），
// 保持到下一次按压前（爬行启动后由爬行帧覆盖，停止爬行回到待机）。
// 方案A：按压换表情、但不爬行；方案C：完全静态。
function showRandomEmotion() {
  if (skin === 'original') return
  setSkinFrame('state', 1 + Math.floor(Math.random() * 4))
}
function setupSkin() {
  if (skin === 'original') {
    img.src = IMG_URL
    ...
    preloadFrames(skin)
    setSkinFrame('state', 0)
  }
  setupHitTest()
}
function setSkin(v) {
  skin = v === 'a' || v === 'b' ? v : 'original'
...
function scaleToDisplay(s) {
  return Math.round((s - MIN_SCALE) / ((MAX_SCALE - MIN_SCALE) / 69)) + 1
}
function setScale(v) {
...
// —— 空闲爬行：最后一次交互结束 N 秒无按压后，鲸鱼沿屏幕四边顺时针绕圈 ——
function resetIdleTimer() {
  if (crawlIdleTimer) { clearTimeout(crawlIdleTimer); crawlIdleTimer = null }
  if (!crawlOn) return
  crawlIdleTimer = setTimeout(startCrawl, crawlIdleMs)
}
// 爬行边界：让「鲸鱼本体中心」沿屏幕边缘内侧一圈行进（肚子贴着每条边），
// 而不是让盒子角压着屏幕角 → 四条边都真正贴边。
// 本体中心相对盒子：img 位于右下 59.45%，本体中心在 img 的 50%/64%。
var BODY_CX_F = 1 - 0.5945 + 0.5945 * 0.5   // ≈0.7028
var BODY_CY_F = 1 - 0.5945 + 0.5945 * 0.64  // ≈0.7860
var BELLY_F = 0.19 // 本体中心到「肚子」的距离（相对盒子边长），肚子贴边用
function crawlBounds() {
  var vp = viewport()
  var w = root.offsetWidth || root.getBoundingClientRect().width || 0
  var h = root.offsetHeight || root.getBoundingClientRect().height || 0
  var cxOff = w * BODY_CX_F
  var cyOff = h * BODY_CY_F
  var belly = Math.max(w, h) * BELLY_F
  return {
    minX: belly - cxOff,
    maxX: vp.w - rightGap() - belly - cxOff,
    minY: belly - cyOff,
    maxY: vp.h - belly - cyOff,
  }
}
function nearestPerimeter(x, y, b) {
  var cx = clamp(x, b.minX, b.maxX)
  var cy = clamp(y, b.minY, b.maxY)
  var dl = x - b.minX
  var dr = b.maxX - x
  var dt = y - b.minY
  var db = b.maxY - y
  var m = Math.min(dl, dr, dt, db)
  if (m === dl) return { x: b.minX, y: cy }
  if (m === dr) return { x: b.maxX, y: cy }
  if (m === dt) return { x: cx, y: b.minY }
  return { x: cx, y: b.maxY }
}
function nextCornerIdx(sx, sy, b) {
  // 顺时针拐角：0=左上 1=右上 2=右下 3=左下
  var eps = 0.5
  if (sy <= b.minY + eps) return sx >= b.maxX - eps ? 2 : 1
  if (sx >= b.maxX - eps) return sy >= b.maxY - eps ? 3 : 2
  if (sy >= b.maxY - eps) return sx <= b.minX + eps ? 0 : 3
  return sy <= b.minY + eps ? 1 : 0
}
function crawlEdge() {
  var b = crawl.b
  var eps = 1
  if (crawl.y <= b.minY + eps) return 'top'
  if (crawl.y >= b.maxY - eps) return 'bottom'
  if (crawl.x <= b.minX + eps) return 'left'
  if (crawl.x >= b.maxX - eps) return 'right'
  return 'bottom'
}
// 爬行姿态：身体贴着当前边缘（肚子朝向边缘、头朝行进方向）。
// 帧图本体中心约在 (50%, 64%)，绕它旋转保证身体原地转向、贴边不漂。
var CRAWL_ORIGIN = '50% 64%'
var crawlAngle = 0
function applyCrawlOrientation() {
  var base = { bottom: 0, left: 90, top: 180, right: 270 }[crawlEdge()] || 0
  // 累加角度避免转角处反向打转（如 180°→-90° 会倒着转 270°）
  crawlAngle = base + Math.round((crawlAngle - base) / 360) * 360
  img.style.transition = 'transform .25s ease'
  img.style.transformOrigin = CRAWL_ORIGIN
  img.style.transform = 'rotate(' + crawlAngle + 'deg)'
}
function startCrawl() {
  if (crawling || !crawlOn || menuOpen || (drag && drag.active)) return
  // 只有方案B 空闲爬行；方案A/方案C 完全静态
  if (skin !== 'b') return
  var b = crawlBounds()
  if (b.maxX - b.minX <= 0 && b.maxY - b.minY <= 0) return
  var p = nearestPerimeter(state.left, state.top, b)
  state.left = p.x
  state.top = p.y
  state.h = null
  state.v = null
  crawl = { b: b, x: p.x, y: p.y, ti: nextCornerIdx(p.x, p.y, b), lastTs: null }
  crawling = true
  crawlAngle = 0
  root.classList.add('dshwv-crawl')
  applyCrawlOrientation()
  if (skin !== 'original') {
    frameIdx = 0
    frameLastTs = 0
    var order0 = CRAWL_ORDER[skin]
    setSkinFrame('crawl', order0 ? order0[0] : 0)
  }
  crawlAnimId = requestAnimationFrame(crawlStep)
}
function stopCrawl() {
  if (!crawling) return
  crawling = false
  root.classList.remove('dshwv-crawl')
  if (crawlAnimId) { cancelAnimationFrame(crawlAnimId); crawlAnimId = null }
  crawl = null
  if (skin !== 'original' && !menuOpen) setSkinFrame('state', 0)
  express()
}
function crawlStep(ts) {
  if (!crawling || !crawl) return
  if (crawl.lastTs === null) crawl.lastTs = ts
  var dt = (ts - crawl.lastTs) / 1000
  crawl.lastTs = ts
  if (!(dt > 0) || dt > 0.1) dt = 0.016
  var dist = crawlSpeed * dt
  var b = crawl.b
  var cx = [b.minX, b.maxX, b.maxX, b.minX]
  var cy = [b.minY, b.minY, b.maxY, b.maxY]
  var guard = 0
  ...
  state.top = crawl.y
  express()
  // 形象帧循环：爬行时按速度节奏切换帧序列（方案A=静态 crawl-0；B=动画；原版=单图）
  if (skin !== 'original' && SKIN_ANIMATED[skin]) {
    var frameMs = Math.max(40, Math.min(250, Math.round(5000 / crawlSpeed)))
    var count = CRAWL_FRAME_COUNT[skin] || 8
    if (frameLastTs === 0) frameLastTs = ts
    if (ts - frameLastTs >= frameMs) {
      frameLastTs = ts
      var order = CRAWL_ORDER[skin]
      if (order && order.length) {
        frameIdx = (frameIdx + 1) % order.length
        setSkinFrame('crawl', order[frameIdx])
      } else {
        frameIdx = (frameIdx + 1) % count
        setSkinFrame('crawl', frameIdx)
      }
    }
  }
  crawlAnimId = requestAnimationFrame(crawlStep)
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) {
    if (remindBubbleActive) hideRemindBubble()
    showBubble()
    refresh(true)
    return
  }
...
window.addEventListener('resize', function () {
  if (crawling) { stopCrawl(); startCrawl(); return }
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.remindOn === 'boolean') {
      remindOn = d.remindOn
      remindToggle.checked = remindOn
    }
    ...
    if (d && typeof d.crawlOn === 'boolean') {
      crawlOn = d.crawlOn
      crawlToggle.checked = crawlOn
      crawlIdleInput.disabled = !crawlOn
    }
    if (d && typeof d.crawlIdleMs === 'number') {
      crawlIdleMs = d.crawlIdleMs > 0 ? Math.round(d.crawlIdleMs) : CRAWL_DEFAULT_IDLE_MS
      crawlIdleInput.value = String(Math.round(crawlIdleMs / 1000))
    }
    if (d && typeof d.crawlSpeed === 'number') {
      crawlSpeed = d.crawlSpeed > 0 ? Math.round(d.crawlSpeed) : CRAWL_SPEED
      crawlSpeedInput.value = String(crawlSpeed)
    }
    if (d && typeof d.skin === 'string' && (d.skin === 'a' || d.skin === 'b' || d.skin === 'original')) {
      skin = d.skin
      skinSelect.value = skin
      setupSkin()
    }
      ...
      .then(function (d) {
        ...
        if (d.seq > lastCostSeq) {
          ...
          if (d.turn !== null && d.amount !== null) {
            // 用户不在页面上 → 弹「常回家看看」提醒；在的话照旧弹本轮消耗
            if (!notifyTurnDone()) showCostBubble(Number(d.amount))
          }
        }
      })
      .catch(function () {})
...
function apply(ctx) {
    ...
    // —— 完成提醒（系统级）：浏览器上报页面是否聚焦；回答完成且不在页面上时
    // 由宿主进程直接发 macOS 原生通知（浏览器气泡在后台看不见，所以必须走系统通知）。
    let pageFocused = true // 默认视为在前台，未收到上报时不打扰
    let lastNotifyAt = 0
    function notifyOS(bodyText, title) {
      if (process.platform !== 'darwin') return
      const esc = (s) => String(s).replace(/\\/g, '\\\\').replace(/"/g, '\\"')
      const script = 'display notification "' + esc(bodyText) + '" with title "' + esc(title) + '" sound name "default"'
      try {
    ...
    function maybeNotifyAway() {
      try {
        ...
        if (cfg.remindOn === false) return
        if (pageFocused) return
        const now = Date.now()
        if (now - lastNotifyAt < 5000) return // 防抖：5 秒内只提醒一次
        lastNotifyAt = now
        notifyOS('常回家看看 🌊 回答已经写完啦', '🐋 DSH 小鲸鱼挂件')
      } catch (err) {}
    }
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              crawlOn: parsed.crawlOn !== false,
              crawlIdleMs: typeof parsed.crawlIdleMs === 'number' ? parsed.crawlIdleMs : 10000,
              crawlSpeed: typeof parsed.crawlSpeed === 'number' ? parsed.crawlSpeed : 45,
              skin: normalizeSkin(parsed.skin) || 'original',
              remindOn: parsed.remindOn !== false,
            }
          }
        } catch (err) {}
      }
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, crawlOn, crawlIdleMs, crawlSpeed, skin, remindOn) {
      ...
      const cro = crawlOn !== false
      const crms = typeof crawlIdleMs === 'number' && crawlIdleMs > 0 ? Math.round(crawlIdleMs) : 10000
      const csp = typeof crawlSpeed === 'number' && crawlSpeed > 0 ? Math.round(crawlSpeed) : 45
      const sk = normalizeSkin(skin) || 'original'
      const rmo = remindOn !== false
    ...
    // 形象帧（spritesheet 切片）：/dsh-whale/frame.png?skin=a|b&type=crawl|state&i=N
    const CRAWL_MAX_INDEX = { a: 7, b: 7 }
    ...
    function frameQuery(url) {
      try {
        ...
        const type = get('type')
        const i = Number(get('i'))
        return {
          skin: normalizeSkin(get('skin')),
          type: type === 'crawl' ? 'crawl' : (type === 'state' ? 'state' : null),
          i: Number.isInteger(i) ? i : -1,
        }
      } catch (err) {
        return { skin: null, type: null, i: -1 }
      }
    }
    const frameCache = new Map()
    function loadFrame(skin, type, i) {
      const max = type === 'crawl' ? (CRAWL_MAX_INDEX[skin] !== undefined ? CRAWL_MAX_INDEX[skin] : 7) : 4
      if (i < 0 || i > max) return null
      const key = skin + '/' + type + '/' + i
      if (frameCache.has(key)) return frameCache.get(key)
      const p = path.join(PACKAGE_ROOT, 'assets', 'frames', 'skin-' + skin, type + '-' + i + '.png')
      try {
        const bytes = fs.readFileSync(p)
        if (bytes && bytes.length > 0) {
          frameCache.set(key, bytes)
          return bytes
        }
      } catch (err) {}
      return null
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/frame.png',
      handler: (req, res) => {
        const q = frameQuery(req.url)
        const bytes = q.skin ? loadFrame(q.skin, q.type, q.i) : null
        if (!bytes) {
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
    ...
    // 页面聚焦状态上报：客户端在 focus/blur/visibilitychange 及心跳时调用，
    // 宿主端据此决定「回答完成时是否发系统通知」（后台标签看不见气泡）
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/presence.json',
      handler: async (req, res) => {
        try {
          if (req.method === 'PUT' || req.method === 'POST') {
            const body = await readBody(req)
            const parsed = JSON.parse(body || '{}')
            pageFocused = parsed.focused !== false
          }
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: true, focused: pageFocused }))
        } catch (err) {
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: false, focused: pageFocused }))
        }
      },
    }))
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.crawlOn, parsed.crawlIdleMs, parsed.crawlSpeed, parsed.skin, parsed.remindOn)
